...
var viewTpl = Handlebars.compile(Mock.heredoc(config.view.docTemplate))
...
module.exports = function (settings) {
    ...
    controller.handle(settings.url, 'view', settings, function (req, res) {
        ...
        if (resType) {
            ...
            extend(true, data, resBody)
        }
        config.view.request(req, data)

        var templateDir = config.root + config.view.templateDir
        var viewSettings = {
            template: templateDir + settings.template,
            data: data,
            templateDir: templateDir,
            templatePluginDir: config.root + config.view.templatePluginDir
        }
        req.body['_fms'] = JSON.stringify(viewSettings)
        request(
            {
                method: 'POST',
                uri: config.view.server + settings.url + '?' + qs.stringify(req.query),
                formData: req.body
            },
